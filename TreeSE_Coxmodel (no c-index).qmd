...
implement cox model on: all pair-wise log ratio transformation/ clr transformation / raw data
...
Cindex_signature <- glmnet::Cindex(pred = predictions4, y_test_rel)
```
...
models <- list(
transformed_apw = list(model = model1, cvfit = cvfit1, x_test = lrX_test, x_train = lrX),
untransformed = list(model = model2, cvfit = cvfit2, x_test = x_test, x_train = x),
clr= list(model = model3, cvfit = cvfit3, x_test = x_test_clr, x_train = x_clr),
rel=list(model = model4, cvfit = cvfit4, x_test = x_test_rel, x_train = x_rel)
)
